      ...
      <div className="container mx-auto px-6 h-20 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8 font-mono text-sm">
          {['Markets', 'Features', 'Pricing', 'Testimonials'].map((item) => (
            <a key={item} href={\`#\${item.toLowerCase()}\`} onClick={(e) => scrollTo(item.toLowerCase(), e)} aria-label={\`Go to \${item}\`} className="text-gray-300 hover:text-white transition-colors relative group py-2">
              {item}
        ...
        <div className="flex flex-col gap-6 font-mono text-sm">
           {['Markets', 'Features', 'Pricing', 'Testimonials'].map((item) => (
            <a 
              ...
              onClick={(e) => scrollTo(item.toLowerCase(), e)}
